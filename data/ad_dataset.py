...
@DATA.register_module
class RDPPAD_RealIAD(data.Dataset):
	def __init__(self, cfg, train=True, transform=None, target_transform=None):
		self.simplexNoise = Simplex_CLASS()
		self.root = cfg.data.root
		self.train = train
	...
	def __getitem__(self, index):
		if self.use_sample == True:
			...
			for i in range(5):
				...
				image = cv2.resize(image / 255., (self.transform.transforms[0].size[0], self.transform.transforms[0].size[1]))
				img.append(image)
				size = self.transform.transforms[0].size[0]
				h_noise = np.random.randint(10, int(size // 8))
				w_noise = np.random.randint(10, int(size // 8))
				...
				init_zero[start_h_noise: start_h_noise + h_noise, start_w_noise: start_w_noise + w_noise,
				:] = 0.2 * simplex_noise.transpose(1, 2, 0)
				image_noise = image + init_zero
				img_noise.append(image_noise)
				if anomaly[i] == 0:
					img_mask.append(Image.fromarray(np.zeros((size, size)), mode='L'))
				else:
				...
				img[i] = self.data_transforms(img[i]) if self.data_transforms is not None else img[i]
				img[i] = img[i].unsqueeze(0)
				img_noise[i] = self.data_transforms(img_noise[i]) if self.data_transforms is not None else img_noise[i]
				img_noise[i] = img_noise[i].unsqueeze(0)
			...
			img_noise = torch.cat(img_noise, dim=0)
			img_mask = torch.cat(img_mask, dim=0)
			return {'img': img, 'img_noise': img_noise, 'img_mask': img_mask, 'cls_name': cls_name, 'anomaly': anomaly, 'img_path': img_path,
					'sample_anomaly': sample_anomaly}
		else:
			data = self.data_all[index]
			img_path, mask_path, cls_name, specie_name, anomaly = data['img_path'], data['mask_path'], data['cls_name'], \
			data['specie_name'], data['anomaly']
...
class DeSTSeg(data.Dataset):
	...
	def __getitem__(self, index):
		if self.train:
			...
			sample = {'img': image, "img_mask": anomaly_mask, 'cls_name': cls_name,
					  'augmented_image': augmented_image, 'anomaly': has_anomaly, 'img_path': img_path}
			return sample
		else:
			data = self.data_all[index]
			img_path, mask_path, cls_name, specie_name, anomaly = data['img_path'], data['mask_path'], data['cls_name'], \
