...
# from util.adeval import EvalAccumulatorCuda
...
class Evaluator(object):
    ...
    def run(self, results, cls_name, logger=None):
        ...
        if 'smp_pre' in results:
            if results['smp_pre'].shape[0] == results['smp_masks'].shape[0]:
                pr_sa_max = results['smp_pre'][idxes]
                gt_sa = results['smp_masks'][idxes]
            else:
